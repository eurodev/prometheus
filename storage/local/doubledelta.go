...
func (c doubleDeltaEncodedChunk) lastTime() clientmodel.Timestamp {
	idx := c.len() - 1
	if idx < 1 {
		// Fast path without creating an iterator.
		return c.firstTime()
	}
	return c.newIterator().getTimestampAtIndex(idx)
}
...
// getValuesAtTime implements chunkIterator.
func (it *doubleDeltaEncodedChunkIterator) getValuesAtTime(t clientmodel.Timestamp) metric.Values {
